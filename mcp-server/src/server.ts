...
function agentSecretUsagePolicy(): Record<string, unknown> {
  return {
    ...
    safeWorkflow: [
      ...
      "For clearly long-running SSH, use ssh_job_start or ssh_batch_job_start with a stable idempotencyKey. Reusing the same key for identical work returns the original operationID; a conflicting operation returns IDEMPOTENCY_KEY_CONFLICT and must not be retried under a new key merely to force execution.",
      "Manage async work with secret_operation_status, cursor-based secret_operation_output, and secret_operation_cancel. Status does not embed stdout/stderr. cancelled is definitive before execution; outcomeUnknown requires reconciling remote state before any retry.",
