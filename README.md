...
- **Long-running work has a public operation lifecycle.** `ssh_job_start` / `ssh_batch_job_start` return an `operationID` immediately and require an idempotency key so retries cannot silently duplicate the same remote effect. Agents use `secret_operation_status`, cursor-based `secret_operation_output`, and `secret_operation_cancel`; states include queued, awaiting approval, running, succeeded, failed, cancelled, and outcome-unknown. Output cursors expose only already-sanitized SSH output.
